...
-   Current topic: !`ls study/.learning/ 2>/dev/null | grep -v scripts`

**Note:** The `study/.learning/` directory is already initialized. Check the topic folder name (ignore `scripts/`).
...
3. Run: `cd study && python3 .learning/scripts/init_learning.py "[topic name]" study/.learning`
4. Parse JSON output and follow `llm_directive`
5. **READ** `study/.learning/<topic-slug>/syllabus.md` to see the template structure
...
8. Update metadata: `"syllabus_generated": true` in `study/.learning/<topic-slug>/metadata.json`
